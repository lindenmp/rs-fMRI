...
% Linden Parkes, BMH, 2016
	...
	if addText == true
		if length(data) == 1 || length(data) == 2
			if size(data{1},2) == 1
				text(XTicks,repmat(yLimits(2) - yLimits(2)*.05,1,size(data{1},1)),num2str(data{1},'%0.1f'),... 
			...
			elseif size(data{1},2) == 2
				offset = [-0.125 0.125];
				for i = 1:2
					text(XTicks+offset(i),repmat(yLimits(2) - yLimits(2)*.05,1,size(data{1},1)),num2str(data{1}(:,i),'%0.1f'),... 
					'HorizontalAlignment','right',... 
					'VerticalAlignment','middle',...
					'Color','black',...
					'FontSize', FSize,...
		...
		if length(data) == 2
			if size(data{2},2) == 1
				text(XTicks,repmat(yLimits(1) - yLimits(1)*.05,1,size(data{2},1)),num2str(data{2},'%0.1f'),... 
			...
			elseif size(data{2},2) == 2
				offset = [-0.125 0.125];
				for i = 1:2
					text(XTicks+offset(i),repmat(yLimits(1) - yLimits(1)*.05,1,size(data{2},1)),num2str(data{2}(:,i),'%0.1f'),... 
					'HorizontalAlignment','left',... 
					'VerticalAlignment','middle',...
					'Color','black',...
					'FontSize', FSize,...
